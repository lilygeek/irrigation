...
class Device:
    ...
    def add_water(self,value, hour, timestamp):
        ...
        if True:
            async with httpx.AsyncClient() as client:
                ...
                for i in result:
                    ...
                    lastTime = max(int(lastTime1),int(lastTime2))


                    dt = datetime.fromtimestamp(lastTime/1000)
            
                    if dt.hour == self.time and dt.minute>30:
                        flag = True
                        year = dt.year
                        month = dt.month
                    #print(dt1,dt2)
                    ...
                    if water_flag == 2:
                        self.add_water(water, dt.hour, lastTime)
                        water_flag = 0
                ...
                if flag:
                    ...
                    state = np.array([month,rpet,pcpn,temp_max,temp_min,temo_mean,humid_max,humid_min,humid_mean,srad,wdsd,water])

                    if args.update:
                        if self.last_state != []:
                            buffering(self.agent,state,action)
                    ...
                    action = inference(self.agent,state)

                    self.action = action

                    msg = EmailMessage()
                    msg.set_content("Please apply " + str(action) + "inches water to potato" + self.name + " now.")
